...
- **I-7 (strengthened; amended 2026-08-04, [doc 22.2](22-realized-sales-venue-comp-data-map.md)):** cross-sourcing must be **structurally independent** — ≥1 confirming source not derived from the venue supplying the primary mark. **Independence is keyed to the underlying transaction CORPUS / ultimate parent, NOT the brand:** eBay owns eBay + TCGplayer + Goldin (= ONE source); PSA APR + Card Ladder share a parent (= ONE). The only eBay-independent card venues are **Heritage** and **Fanatics/PWCC**.
